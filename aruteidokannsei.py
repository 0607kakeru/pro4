...
class SelectScreenToChangeOrDelete:
    def __init__(self, parent, row, col):
        ...
        delete_button = tk.Button(
            self.dialog,
            text="削除",
            command=lambda: self.show_delete_confirmation_dialog(row, col),
        )
...
class SelectScreenToChangeDeleteOrMove:
    def __init__(self, parent, row, col):
        ...
        move_button = tk.Button(
            self.dialog,
            text="提出済みに移動",
            command=lambda: self.move_assignment_to_submitted(row, col),
        )
    ...
    def show_delete_confirmation_dialog(self):
        DeleteConfirmationDialog(self.dialog, self.row, self.col, self)
...
class TimetableScreen:
    ...
    def show_delete_confirmation_dialog(self, row, col, assignment_options_dialog=None):
        DeleteConfirmationDialog(self.root, row, col, self, assignment_options_dialog)
    ...
    def update_submitted_data(self):
        ...
        for assignment in self.submitted_data:
            text = f"{assignment['assignment']} - {assignment['deadline']}"
    ...
    def show_edit_assignment_dialog(self, row, col):
        ...
        if assignment_input_dialog.ok_clicked:
            ...
            # 変更内容を反映するためにセルの内容を更新
            self.labels[row][col].configure(text=assignment_name + "\n" + deadline)

    def change_subject(self, row, col, subject_name):
        self.timetable.data[row][col]["subject"] = subject_name
        self.labels[row][col].configure(text=subject_name)
        self.timetable.save_data()
    
    
    def change_assignment(self, row, col, assignment_name, deadline):
        self.timetable.data[row][col]["assignment"] = assignment_name
        self.timetable.data[row][col]["deadline"] = deadline
    ...
    def return_assignment_to_timetable(
        ...
        self.update_submitted_data_label()  # 提出済み欄を更新
